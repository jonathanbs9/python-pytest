...
class LoadScheduling:
    ...
    This distributes the test collected across all nodes so each test
    ...
    scheduler to assign more test if the number of pending test for
    ...
    :node2pending: Map of nodes and the indices of their pending
       test.  The indices are an index into ``.pending`` (which is
    ...
    :pending: List of indices of globally pending test.  These are
       test which have not yet been allocated to a chunk for a node
       to process.
    ...
    def tests_finished(self):
        """Return True if all test have been executed by the nodes."""
    ...
    def add_node(self, node):
        ...
        From now on the node will be allocated chunks of test to
        execute.
    ...
    def check_schedule(self, node, duration=0):
        ...
        check if the given node should be given any more test.  The
        ``duration`` of the last test is optionally used as a
        heuristic to influence how many test the node is assigned.
        """
        ...
        if self.pending:
            ...
            if len(node_pending) < items_per_node_min:
                if duration >= 0.1 and len(node_pending) >= 2:
                    # seems the node is doing long-running test
    ...
    def schedule(self):
        ...
        if not self._check_nodes_have_same_collection():
            self.log("**Different test collected, aborting run**")
            return
        ...
        # Send a batch of test to run. If we don't have at least two
        # test per node, we have to send them all so that we can send
        # shutdown signals and get all nodes working.
        initial_batch = max(len(self.pending) // 4, 2 * len(self.nodes))

        # distribute test round-robin up to the batch size
        # (or until we run out)
        ...
        if not self.pending:
            # initial distribution sent all test, start node shutdown
            for node in self.nodes:
                node.shutdown()
