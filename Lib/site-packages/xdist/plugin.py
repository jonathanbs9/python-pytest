...
def pytest_addoption(parser):
    ...
    group.addoption(
        ...
        action="store",
        type=int,
        help="limit the maximum number of workers to process the test when using --numprocesses=auto",
    )
    group.addoption(
        "--max-worker-restart",
        action="store",
    ...
    group.addoption(
        ...
        default="no",
        help=(
            "set mode for distributing test to exec environments.\n\n"
            "each: send each test to all available environments.\n\n"
            "load: load balance by sending any pending test to any"
            " available environment.\n\n"
            "loadscope: load balance by sending pending groups of test in"
            " the same scope to any available environment.\n\n"
            "loadfile: load balance by sending test grouped by file"
            " to any available environment.\n\n"
            "(default) no: run test inprocess, don't distribute."
        ),
    )
    group.addoption(
        "--tx",
    ...
    group._addoption(
        ...
        dest="distload",
        default=False,
        help="load-balance test.  shortcut for '--dist=load'",
    )
    group.addoption(
        "--rsyncdir",
        action="append",
...
def pytest_cmdline_main(config):
    ...
    if not val("collectonly") and val("dist") != "no" and usepdb:
        raise pytest.UsageError(
            "--pdb is incompatible with distributing test; try using -n0 or -nauto."
        )  # noqa: E501
...
def get_xdist_worker_id(request_or_session) -> str:
    ...
    If not distributing test (for example passing `-n0` or not passing `-n` at all)
    also return 'master'.
